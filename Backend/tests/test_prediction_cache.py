...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_repository_get_cached_prediction_calls_storage() -> None:
    storage = AsyncMock()
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_repository_cache_prediction_calls_storage() -> None:
    storage = AsyncMock()
    repo = ModerationRepository(InMemoryStorage())
    repo.configure_cache_storage(storage)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_repository_cache_methods_without_storage_do_not_fail() -> None:
    repo = ModerationRepository(InMemoryStorage())
...
@pytest.mark.asyncio
async def test_redis_prediction_cache_roundtrip() -> None:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_redis_prediction_cache_ttl_expiration() -> None:
    redis_url = "redis://localhost:6379/15"
    client = Redis.from_url(redis_url, decode_responses=True)
    try:
        await client.ping()
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_redis_prediction_cache_delete() -> None:
    """Test that delete_prediction removes entry from Redis."""
    redis_url = "redis://localhost:6379/15"
    client = Redis.from_url(redis_url, decode_responses=True)
    try:
        await client.ping()
    ...
    storage = RedisPredictionCacheStorage(client, ttl_seconds=30)
    key = "predict:test:delete"

    try:
        await client.delete(key)
        await storage.set_prediction(
            key, PredictResponse(is_violation=True, probability=0.75)
        )
        assert await storage.get_prediction(key) is not None
        
        await storage.delete_prediction(key)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_repository_delete_cached_prediction_calls_storage() -> None:
    """Test that delete_cached_prediction delegates to storage."""
    storage = AsyncMock()
    repo = ModerationRepository(InMemoryStorage())
    repo.configure_cache_storage(storage)

    await repo.delete_cached_prediction("predict:delete:key")

    storage.delete_prediction.assert_awaited_once_with("predict:delete:key")
